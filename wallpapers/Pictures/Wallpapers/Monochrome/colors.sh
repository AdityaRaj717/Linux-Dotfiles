...
# ---------------------------------------------------------
# Give files a moment to settle
sleep 0.2

# Reload Cava (Forces config reload)
# Try pkill first, then killall if that fails
pkill -USR1 cava || killall -USR1 cava

# Note: BTOP does not support live reload. It must be restarted manually.
notify-send "Theme" "Graphite Mono Applied. (Please restart btop)"
